classdef TCGAUtils
    ...
    methods (Static = true, Access = public)
        function [vsCentreIDs, vsPatientIDs, vsSlideIDs, vsTileIDs, vsFilenames] = GetIDsFromTileFilepaths(vsTileFilepaths, NameValueArgs)
            ...
            %[vsCentreIDs, vsPatientIDs, vsSlideIDs, vsTileIDs, vsFilenames] = TCGAUtils.GetIDsFromTileFilepaths(vsTileFilepaths,'bSlideNamesNotTilesGiven', true)
            arguments
                vsTileFilepaths
    ...
    methods (Static)
        function [vbTrainSlideIndices, vbTestSlideIndices] = PerformRandomTwoWaySplit(vsSlideNames,dFractionGroupsInTraining, NameValueArgs)
            ...
            if dNumGroups < 2
               error(['You cannot split by ', chGroupName, ' because there is only one unique group within that.'])
            end
