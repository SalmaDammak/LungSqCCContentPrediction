classdef (Abstract) ErrorMetricsCalculator
    ...
    methods (Access = public, Static = true)
        ...
        function dPPV = CaluclatePositivePredictiveValue(viTruth, vdConfidences, iPositiveLabel, dThreshold)
            vdPredictions = vdConfidences >= dThreshold;
            dNumTruePositive = 0;
            dNumFalsePositive = 0;
            
            for i = 1:length(vdPredictions)
                if vdPredictions(i) == iPositiveLabel && viTruth(i) == iPositiveLabel
                    dNumTruePositive = dNumTruePositive + 1;
                elseif vdPredictions(i) == iPositiveLabel && viTruth(i) ~= iPositiveLabel
        ...
        function dNPV = CaluclateNegativePredictiveValue(viTruth, vdConfidences, iPositiveLabel, dThreshold)
            vdPredictions = vdConfidences >= dThreshold;
            dNumTrueNegative = 0;
            dNumFalseNegative = 0;
            ...
            for i = 1:length(vdPredictions)
                if vdPredictions(i) ~= iPositiveLabel && viTruth(i) ~= iPositiveLabel
                    dNumTrueNegative = dNumTrueNegative + 1;
                elseif vdPredictions(i) ~= iPositiveLabel && viTruth(i) == iPositiveLabel
                    dNumFalseNegative = dNumFalseNegative + 1;
                end
            end
            dNPV = dNumTrueNegative / (dNumTrueNegative + dNumFalseNegative);
        end
